...
<body>
        ...
        <div class="" style="margin: auto;width: 75%;">
            <div class=" row">
                ...
                <div class="col-sm-6 col-md-6 col-lg-6">
                    <img style="width: 100% !important;" src="/product/{{ $product->image }}" alt="">
                </div>
                <div class=" col-sm-6 col-md-6 col-lg-6" style="font-weight: bold;padding: 12px">
                    <h1 style="font-size: 40px">
                        {{ $product->title }}
                    </h1>
                    @if ($product->discount_price != null)
                        <h6 class="text-danger" style="font-size: 20px;padding: 12px">
                            - -Giảm còn
                    ...
                    @endif
                    <p style="font-size: 24px;padding: 12px">
                        - -Danh mục: {{ $product->category }}
                    </p>
                    <h6 style="font-size: 20px;padding: 12px">
                       +    + Mô tả: {{ $product->description }}
                    </h6>
                    <h6 style="font-size: 20px;padding: 12px">
                        - -Số lượng hàng: {{ $product->quantity }}
                    </h6>
    ...
    <div style="padding-left: 20%">
        ...
        @foreach ($comment_product as $singleComment)
            @if ($singleComment->product_id == $product->id)
                <div>
            ...
            @foreach ($reply as $rep)
                @if ($rep->comments_product_id == $singleComment->id && $rep->product_id == $product->id)
        ...
        <span style="padding-top: 20px">
            {!! $comment_product->withQueryString()->links('pagination::bootstrap-5') !!}
        </span>



        <div style="display: none" class="replyDiv">
            <form action="{{ url('add_reply_product', $product->id) }}" method="POST">
                @csrf
